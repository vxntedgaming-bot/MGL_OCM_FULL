...
Assigned managers are sent from `/` to the Manager Hub. Public visitors keep the simplified homepage. Club squads are at `/clubs/<club-name-slug>/` (short codes such as `/clubs/ARS/` still resolve). Public News has two destinations: **Pressroom** (`/news/pressroom/`) for approved interviews, and **Live Activity** (`/news/activity/`) for approved results, completed transfers and signings. Manager submissions stay pending until Admin approves them. Set `DISCORD_INVITE_URL` to show Discord buttons (header **DISCORD**, footer **JOIN OUR DISCORD**); leave it empty to hide them. Do not hardcode an invite.
