...
def open_pack(manager,pack,team):
    ...
    def pick(tier, exclude):
        qs=[p for p in Player.objects.filter(is_free_agent=True).order_by("id") if player_tier(p.overall)==tier and p.id not in exclude]
    ...
    if pack.pack_type in ["GOLD","SILVER","BRONZE"]: picks=[pick(pack.pack_type,owned)]
        ...
        qs=[p for p in Player.objects.filter(is_free_agent=True) if p.age and p.age<20 and player_tier(p.overall)=="GOLD" and p.id not in owned]
