...
class NavigationDropdownTests(TestCase):
    ...
    def test_homepage_renders_all_dropdowns(self):
        ...
        self.assertIn('data-nav-dropdown="players"', html)
        ...
        self.assertContains(response, "Cups (Coming soon)")
        ...
        self.assertContains(response, "Player Statistics")
        self.assertContains(response, "Club Statistics")
    ...
    def test_public_nav_omits_private_destinations(self):
        ...
        self.assertIn("PLAYERS", nav)
        self.assertIn("TRANSFERS", nav)
        self.assertIn("AUCTIONS", nav)
        self.assertIn("SCOUTING", nav)
        self.assertIn("NEWS", nav)
        self.assertIn("PRESS", nav)
        self.assertIn("JOBS", nav)
        self.assertIn("RULES", nav)
    ...
    def test_market_dropdown_highlights_current_page(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, 'data-nav-dropdown="transfers"')
    ...
    def test_context_marks_open_menu_for_request(self):
        ...
        self.assertTrue(menus["players"]["is_current"])
        current_labels = [
            item["label"] for item in menus["players"]["items"] if item["is_current"]
        ]
        self.assertEqual(current_labels, ["Free Agents"])
        self.assertNotIn("career", menus)
    ...
    def test_assigned_manager_gets_manager_nav(self):
        ...
        self.assertContains(hub, "Player Statistics")
    ...
    def test_signed_in_nav_shows_tables_and_stats_for_every_role(self):
        ...
        self.assertIn("SCOUTING", public_nav)
