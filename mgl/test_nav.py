...
class NavigationDropdownTests(TestCase):
    ...
    def test_homepage_renders_all_dropdowns(self):
        ...
        self.assertNotIn('data-nav-dropdown="clubs"', html)
        self.assertNotIn('data-nav-dropdown="players"', html)
        self.assertNotIn('data-nav-dropdown="transfers"', html)
        self.assertNotIn('data-nav-dropdown="news"', html)
        self.assertNotIn('data-nav-dropdown="stats"', html)
        ...
        self.assertNotContains(response, reverse("fixture_list"))
        self.assertEqual(self.client.get(reverse("job_centre")).status_code, 200)
        jobs = reverse("job_centre")
        self.assertEqual(self.client.get(reverse("fixture_list")).url, jobs)
        self.assertEqual(self.client.get(reverse("clubs_index")).status_code, 200)
        leagues = self.client.get(reverse("leagues_page"))
        self.assertContains(leagues, 'data-nav-dropdown="leagues"')
        self.assertContains(leagues, "data-nav-trigger")
        self.assertContains(leagues, "mgl-nav-chevron")
    ...
    def test_my_club_dropdown_highlights_when_logged_in(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, 'data-nav-dropdown="career"')
    ...
    def test_assigned_manager_gets_manager_nav(self):
        ...
        self.assertIn('data-nav-dropdown="career"', html)
        self.assertIn('data-nav-dropdown="market"', html)
        self.assertIn('data-nav-dropdown="stats"', html)
        self.assertIn('data-nav-dropdown="history"', html)
        self.assertIn('data-nav-dropdown="cups"', html)
        self.assertContains(hub, "MY TEAM")
        self.assertContains(hub, "MARKET")
        self.assertContains(hub, "Scouting")
        self.assertContains(hub, "Youth Academy")
        self.assertNotContains(hub, "Head To Head")
        self.assertContains(hub, "Hall of Fame")
        self.assertContains(hub, "UFL Newsroom")
        self.assertContains(hub, "UFL Press Conference")
        ...
        self.assertContains(hub, "/market/youth-academy/")
        ...
        self.assertContains(hub, "JOB CENTRE")
        ...
        self.assertContains(hub, "LEAGUE")
        ...
        self.assertNotContains(hub, 'data-nav-dropdown="my-club"')
    ...
    def test_signed_in_nav_shows_tables_and_stats_for_every_role(self):
        ...
        for username in ("navuser", "navadmin", "navowner2"):
            ...
            if username == "navuser":
                self.assertEqual(page.status_code, 302, username)
        ...
        self.assertIn("LEAGUE", public_nav)
        self.assertNotIn("SCOUTING", public_nav)
        self.assertNotIn("MY CLUB", public_nav)
        self.assertNotIn("MY CAREER", public_nav)
...
class PlayerSearchAndCardNameTests(TestCase):
    ...
    def test_free_agents_page_uses_table_layout_and_group_filters(self):
        ...
        self.assertIn(reverse("job_centre"), guest["Location"])
