...
def manager_hub(request):
    ...
    if team:
        ...
        recent=Fixture.objects.filter(Q(home_team=team) | Q(away_team=team)).order_by("-id")[:10]
...
def submit_match(
    ...
    if MatchSubmission.objects.filter(
        fixture=fixture
    ).exists():
    ...
    if request.method == "POST":
        ...
        for team in [
            ...
            for pid in request.POST.getlist(
                prefix + "goal_players"
            ):
...
def club_management_admin(request):
    """
    Main Owner/Admin club management page.
    """
...
def edit_club_admin(request, team_id):
    """
    Edit club identity without touching the squad.
    """
...
def remove_club_manager(request, team_id):
    """
    Remove the current manager from the club.

    IMPORTANT:
    The club, squad, fixtures, results and history remain intact.
    """
...
def change_club_manager(request, team_id):
    """
    Assign an approved manager to an existing club.
    """
    ...
    # Approved managers who aren't currently managing another club.
    ...
    for application in approved_applications:
        ...
        # Don't show users already managing another club.
    ...
    if request.method == "POST":
        ...
        # Prevent one manager controlling multiple clubs.
        ...
        if old_manager:
            messages.success(
                request,
                f"{team.name} is now managed by "
                f"{new_manager.username}. "
...
def club_squad_admin(request, team_id):
    """
    Owner/Admin view of the complete club squad.
    """
...
def team_management(request):
    ...
    total_ovr = sum(
        player.overall for player in players
    )

    available_spaces = max(
        0,
        team.roster_limit - len(players)
    )
